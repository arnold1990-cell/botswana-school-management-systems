...
class HrService {
    ...
    SubjectExperienceResponse addSubjectExperience(Long staffId, SubjectExperienceRequest r, String actor){ var st=reqEducator(staffId); ensureSchoolReference(r.schoolId(), null, null, r.subjectId()); if(!st.getSchoolId().equals(r.schoolId())) throw new ConflictException("School mismatch"); var e=new EducatorSubjectExperience(); e.setSchoolId(r.schoolId()); e.setStaffId(staffId); e.setSubjectId(r.subjectId()); e.setYearsExperience(r.yearsExperience()); e.setNotes(r.notes()); return map(expRepo.save(e)); }
    ...
    RegisterClassAssignmentResponse assignRegisterClass(RegisterClassAssignRequest r, String actor){ reqEducator(r.staffId()); ensureSchoolReference(r.schoolId(), r.academicYearId(), r.classRoomId(), null); var a=new RegisterClassAssignment(); a.setSchoolId(r.schoolId()); a.setStaffId(r.staffId()); a.setClassRoomId(r.classRoomId()); a.setAcademicYearId(r.academicYearId()); a.setRole(r.role()); return map(registerRepo.save(a)); }
    ...
    private AttendanceResponse createAttendanceRecord(Long schoolId, LocalDate date, Long staffId, StaffAttendanceStatus status, String notes, String actor){
        reqStaff(staffId); if(attendanceRepo.existsBySchoolIdAndStaffIdAndDate(schoolId,staffId,date)) throw new ConflictException("Attendance already captured for staff/date");
